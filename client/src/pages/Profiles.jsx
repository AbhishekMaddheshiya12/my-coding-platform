import { Avatar, Box, colors, Divider, Grid2, Icon, Paper, Typography } from '@mui/material'
  ...
  return (
    <Box sx={{height:'100vh'}}>
        ...
        <Grid2 container spacing={3} sx={{ margin:'auto',marginTop:'10px'}}>
          <Grid2 bgcolor={'#1A2B4A'} size={{xs:6,md:3}}sx={{height:'calc(100vh - 64px)', borderRadius:'10px',display:'flex',flexDirection:'column',transition:'transform 0.3s ease-in-out','&:hover':{transform:'scale(1.015)'}}}>
            {/* profile */}
            ...
            <Box mt={3}>
              ...
              <Typography sx={{fontSize:'15px',color:'white',textAlign:'center'}}>Joined: {formattedDate}</Typography>
          ...
          <Grid2 size={{xs:6,md:9}}sx={{height:'calc(100vh - 64px)', overflowY:'scroll' ,'::-webkit-scrollbar': { display: 'none' }}}>
            <Box>
            <Paper sx={{
                bgcolor: "#2c3e5d", 
                borderRadius: 3,
                ...
                ProblemSolved.map((problem) => {
                  return (
                    <Link key={problem.id} to={`/problems/${problem.id}`}>
                      <Paper sx={{backgroundColor:'#2c3e5d',padding:2,marginTop:2,borderRadius:'5px',color:'white',display:'flex',justifyContent:'space-between'}}>
                        <Typography>{problem.id} . {problem.title}</Typography>
                        <Typography sx={{color:problem.difficulty == 'Easy' ? 'green' : problem.difficulty == 'Medium' ? 'yellow' : 'red'}}>{problem.difficulty}</Typography>
                      </Paper>
                    </Link>
                  )
                })
